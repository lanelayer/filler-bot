...
impl IntentContract {
    ...
    pub fn encode_intent_locker_call(&self, intent_id: B256) -> String {
        // Function selector: intentLocker(bytes32)
        let function_selector = "0x8f4f8f4f"; // Placeholder - would be actual selector
        let intent_id_hex = format!("{:064x}", intent_id);
        format!("{}{}", function_selector, intent_id_hex)
    }

    /// Encode function call for lockIntentForSolving(intentId)
    pub fn encode_lock_intent_call(&self, intent_id: B256) -> String {
        // Function selector: lockIntentForSolving(bytes32)
        let function_selector = "0x12345678"; // Placeholder - would be actual selector
        let intent_id_hex = format!("{:064x}", intent_id);
        format!("{}{}", function_selector, intent_id_hex)
    }

    /// Encode function call for solveIntent(intentId, blockNumber)
    pub fn encode_solve_intent_call(&self, intent_id: B256, block_number: u64) -> String {
        // Function selector: solveIntent(bytes32, uint64)
        let function_selector = "0x87654321"; // Placeholder - would be actual selector
        let intent_id_hex = format!("{:064x}", intent_id);
        let block_number_hex = format!("{:016x}", block_number);
        format!("{}{}{}", function_selector, intent_id_hex, block_number_hex)
    }
...
mod tests {
    ...
    fn test_function_encoding() {
        ...
        let call = contract.encode_lock_intent_call(intent_id);
        assert!(call.starts_with("0x12345678"));

        // Test solve intent call
        let call = contract.encode_solve_intent_call(intent_id, 12345);
        assert!(call.starts_with("0x87654321"));
    }
}
